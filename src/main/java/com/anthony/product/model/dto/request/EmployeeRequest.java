package com.anthony.product.model.dto.request;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Builder;
import lombok.Getter;
import lombok.Setter;

import javax.validation.constraints.NotBlank;
import javax.validation.constraints.NotNull;
import javax.validation.constraints.Size;

@Getter
@Setter
@Builder
public class EmployeeRequest {

    @NotBlank(message = "name field not should be null or empty")
    @Size(min = 5, max = 64, message = "name field should be minimum size 5 and maximum 64")
    @Schema(name = "name", description = "Name of Employee", required = true, example = "Anthony")
    private String name;

    @NotNull(message = "age field not should be null or empty")
    @Schema(example = "1", description = "Age of Employee")
    private int age;
}
